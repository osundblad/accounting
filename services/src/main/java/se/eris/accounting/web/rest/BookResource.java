...
public class BookResource {
    ...
    public RestBook get(@PathVariable("bookId") final UUID bookId) {
        final Book book = bookRestFacade.getAllBooks().filter(b -> b.getId().asUUID().equals(bookId)).findFirst().orElseThrow(() -> new NotFoundException("no " + Book.class.getSimpleName() + " with id " + bookId + " found."));
    ...
    public RestBook getTemplate() {
        return new RestBook(new Book(BookId.empty(), "", ""));
    }
    ...
    public RestBook createRandom(@RequestParam("name") final String name, @RequestParam("description") final String description) {
        return new RestBook(bookRestFacade.create(new Book(BookId.empty(), name, description)));
    }
